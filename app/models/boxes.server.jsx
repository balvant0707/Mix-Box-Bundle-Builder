...
async function getUniqueBoxCode() {
  ...
  do {
    ...
    exists = await db.box.findFirst({ where: { boxCode: code } });
  } while (exists);
  return code;
}
...
async function getRequestedBoxCode(rawValue, excludeId = null) {
  ...
  if (excludeId) {
    const existingRecord = await db.box.findUnique({
...
export async function createBox(shop, data, admin) {
  ...
  return db.box.findUnique({
    where: { id: box.id },
  });
}

export async function updateComboStepsConfig(id, shop, comboStepsConfig) {
  await db.box.findFirstOrThrow({ where: { id: parseInt(id), shop, deletedAt: null } });
...
export async function updateBox(id, shop, data, admin = null) {
  ...
  return db.box.findUnique({ where: { id: parseInt(id) } });
}
...
async function getNextSortOrder(shop) {
  const last = await db.box.findFirst({
