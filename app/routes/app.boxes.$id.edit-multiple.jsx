...
async function loadPickerPage(admin, resource, after = null) {
  ...
  if (json?.errors?.length) {
    console.warn(`[app.multiplebox] ${resource} GraphQL errors`, json.errors);
    return {
      resource,
      items: [],
...
function getDiscountSubmissionFields(form, source = form) {
  ...
  if (source?.discountMode === 'flat_discount') {
    return {
      ...
      discountType: source.discountType === 'fixed_amount' ? 'fixed' : 'percent',
...
export default function EditMultipleMixMatchBundlePage() {
  const {
    initialData,
